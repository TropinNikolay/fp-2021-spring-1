...
lazyExample = do
    ...
    -- потом пролизводится переданное действие над хэндлом, а потом происходит закрытие хэндла.
    --
    -- Опасность тут в том, что закрытие хэндла происходит __энергично__ (то есть неленивно), то есть после вызова
    ...
    --
    -- Обращаю вниамние на то, почему оборачивание всего withFile в forceEvalIO не решает проблему:
    -- в forceEvalIO передаётся пусть и ленивый, но __результат вызова__ withFile. Как мы поняли, 
    -- если вызов withFile произошёл, то хэндл __уже закрыт__. А вот считыание файла ещё не началось
...
catchExample = do
    ...
    -- потом всё кпадёт с ошибкой. Несмотря на наличие catch
    --
    c <- withFile filePath ReadMode $ \h -> forceEvalIO (do
        contents <- hGetContents h
